...
export class CSVProcessor {
     ...
     */
    static extractTradeHistory(csvContent) {
     ...
     */
    static validateCSVFormat(csvContent) {
        try {
            ...
            const missingColumns = requiredColumns.filter(col => !headers.some(header => header.includes(col)));
     ...
     */
    static getTradePreview(csvContent, maxRows = 5) {
        const extractedContent = this.extractTradeHistory(csvContent);
        const lines = extractedContent.trim().split('\n');
        const headers = lines[0].split(',').map(h => h.trim());
        const dataLines = lines.slice(1);
        const previewRows = dataLines.slice(0, maxRows).map(line => line.split(',').map(cell => cell.trim()));
        return {
            headers,
            rows: previewRows,
     ...
     */
    static cleanCSVData(csvContent) {
     ...
     */
    static createPreviewData(trades) {
