...
export class TradeAnalyzer {
    static parseDateTime(dateTimeStr) {
        try {
            ...
            return new Date(fullYear, parseInt(month) - 1, // Month is 0-indexed
    ...
    static analyzeTrades(rawTrades) {
        ...
        for (const symbol of symbols) {
            ...
            const openLongPositions = [];
            const openShortPositions = [];
    ...
    static convertToDatabaseFormat(analyzedTrades, brokerId) {
        return analyzedTrades.map(trade => ({
            symbol: trade.symbol,
            direction: trade.direction,
            quantity: trade.quantity,
            entryDate: trade.entryTime,
            entryTime: trade.entryTime.toTimeString().split(' ')[0], // HH:MM:SS format
            entryPrice: trade.entryPrice,
            exitDate: trade.exitTime || new Date('2001-01-01'),
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            try {
                trades.push({
                    ...
                    side: cells[sideIndex],
                    qty: cells[qtyIndex],
                    posEffect: cells[posEffectIndex],
